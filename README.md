...
Рабочие интерфейсные маршруты: `/dashboard`, `/results`, `/orders`, `/clients`, `/catalog`. Корень перенаправляет на `/dashboard`.

Раздел [«Результаты»: формулы, цены, ограничения и запуск](docs/RESULTS.md) связывает завершённые продажи, прогноз закупок и рекомендации по мероприятиям. Перед запуском новой версии примените миграции; старые цены не восстанавливаются автоматически.
