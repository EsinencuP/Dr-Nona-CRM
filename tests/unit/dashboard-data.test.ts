...
import { aggregateDashboard, dashboardDelta, getPreviousPeriodStats, normalizeRange, startDateForRange, type DashboardOrder } from "../../src/app/(crm)/dashboard/_components/dashboard-data";
...
const now = new Date("2026-09-12T12:00:00Z"); const DAY = 86400000;
function order(age: number, status = "DONE", type = "order"): DashboardOrder { return { createdAt: new Date(now.getTime() - age * DAY), status, type, utmSource: null, client: { region: "Кишинёв" }, items: [{ productSlug: "solaris", quantity: 3, retailPriceAtPurchase: 100.1, distributorPriceAtPurchase: 60.05 }] }; }
function calculate(orders: DashboardOrder[], range: DashboardRange = "30d") { return aggregateDashboard({ orders, range, now, newClients: 4, previousNewClients: 2, statuses: [{ status: "NEW", count: 50 }, { status: "PROCESSING", count: 15 }, { status: "DELIVERY", count: 8 }, { status: "CANCELLED", count: 7 }], recentOrders: [], productNames: new Map([["solaris", "Solaris"]]) }); }
describe("dashboard period and cohort integrity", () => {
  test.each<[DashboardRange, number]>([["7d", 7], ["30d", 30], ["90d", 90]])("%s compares equal rolling windows with half-open boundaries", (range, days) => {
  ...
  test("every reporting widget respects period; operational counts remain global", () => {
    const old = order(65); old.utmSource = "old-source"; old.client.region = "Бельцы";
    const stats = calculate([old, order(1), order(2, "NEW"), order(3, "DONE", "consultation")]);
    expect(stats.kpis.total.value).toBe(3); expect(stats.kpis.done.value).toBe(2);
    expect(stats.kpis.revenue.value).toBe(300.3); expect(stats.kpis.unitsSold.value).toBe(3);
    expect(stats.kpis.profit.value).toBe(120.15); expect(stats.kpis.aov.value).toBe(300.3);
    expect(stats.kpis.conversionRate.value).toBeCloseTo(200 / 3);
    expect(stats.kpis.new).toBe(50); expect(stats.kpis.processing).toBe(15);
  ...
  test("unknown retail is excluded with disclosure and monetary deltas disabled", () => {
    const missing = order(2); missing.items[0].retailPriceAtPurchase = 0;
    const stats = calculate([order(1), missing, order(35)]);
    expect(stats.kpis.revenue).toEqual({ value: 300.3, delta: null, deltaAbs: null });
    expect(stats.kpis.unitsSold.value).toBe(6); expect(stats.quality.missingRetail).toBe(1);
  ...
  test("missing purchase cost blocks profit comparison but retains known revenue", () => {
    const missing = order(1); missing.items[0].distributorPriceAtPurchase = 0;
    const stats = calculate([missing, order(35)]);
    expect(stats.quality.missingCost).toBe(1); expect(stats.kpis.profit.delta).toBeNull();
    expect(stats.kpis.revenue.delta).toBe(0); expect(stats.kpis.revenue.value).toBe(300.3);
  });
  test("cancellations are separate potential revenue, not completed sales", () => {
    const stats = calculate([order(1, "CANCELLED"), order(2)]);
    expect(stats.kpis.cancelled.value).toBe(1); expect(stats.lostRevenue).toBe(300.3);
    expect(stats.kpis.revenue.value).toBe(300.3); expect(stats.kpis.unitsSold.value).toBe(3);
  });
  test("Moldova day and hour grouping does not depend on server timezone", () => {
    const row = order(2); row.createdAt = new Date("2026-09-10T22:30:00Z");
  ...
  test("zero-filled timeline, 24 hours, safe zero denominators and invalid ranges", () => {
    ...
    expect(stats.peakHours).toHaveLength(24); expect(stats.timeline.length).toBeGreaterThanOrEqual(30);
    expect(stats.kpis.revenue).toEqual({ value: 0, delta: null, deltaAbs: 0 });
    expect(stats.kpis.aov.value).toBe(0); expect(stats.kpis.conversionRate.value).toBe(0);
  ...
  test("top 5 sorted by completed units and unknown slugs are preserved", () => {
    const orders = Array.from({ length: 7 }, (_, i) => { const row = order(i + 1); row.items[0].productSlug = `slug-${i}`; row.items[0].quantity = i + 1; return row; });
  ...
  test("donut and legend conserve every source, including null and Other name collisions", () => {
    const grouped = groupSources([{ source: null, count: 9 }, { source: "Другие", count: 8 }, { source: "b", count: 7 }, { source: "c", count: 6 }, { source: "d", count: 5 }, { source: "e", count: 4 }]);
    expect(grouped).toHaveLength(5); expect(grouped.reduce((sum, row) => sum + row.count, 0)).toBe(39);
    expect(grouped[0].label).toBe("Прямой переход"); expect(grouped[4].count).toBe(9);
